...
int main() {
  ...
  for (int tt = 0; tt < t; tt++) {
    ...
    for (int i = 0; i < q; i++) {
      ...
      if (o == 1) {
        cin >> w;
        lct.apply(x, y, {a * w, false});
      } else {
        link_cut_tree::data d = lct.query(x, y);
        while (d.m >= 0) {
          lct.apply(d.v, d.v, {0, true});
          d = lct.query(x, y);
        }
        cout << d.l << "\n";
      }

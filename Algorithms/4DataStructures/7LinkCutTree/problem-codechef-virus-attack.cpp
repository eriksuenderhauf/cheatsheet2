...
// !!! the link cut tree has been changed to support subtree sum queries !!!
// !!! do not simply replace with new version, take care about expose and link !!!
...
int main() {
  ...
  link_cut_tree lct(n);
  for (int i = 0; i < n; i++)
    cin >> lct.v[i].d.s, lct.v[i].t = lct.v[i].d;
  for (int i = 0; i < n - 1; i++) {
    cin >> x >> y, x--, y--;
    lct.link(x, y);
  }
  ...
  for (int i = 0; i < q; i++) {
    ...
    if (t == 1) {
      ...
      if (s[x])
        ...
        if (p[x] >= 0)
          lct.cut(x);
        link_cut_tree::data d = lct.query(x, x);
        cout << d.s << "\n";
        if (p[x] >= 0)
          lct.link(x, p[x]);
      }

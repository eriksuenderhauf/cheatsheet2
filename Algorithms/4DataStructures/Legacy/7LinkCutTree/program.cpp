...
node * getRight(node * x) {
  ...
  if (x->r == 0)
    return 0;
  return getLeftmost(x->r);
}

node * splitLeft(node * x) {
  splay(x);
  node * l = x->l;
  x->l = 0;
  if (l != 0)
...
void insertLeft(node * x, node * v) {
  node * l = splitLeft(x);
  merge(v, x, false);
  merge(l, v, false);
}

void insertRight(node * x, node * v) {
  node * r = splitRight(x);
  merge(x, v, true);
  merge(v, r, true);
}
...
data query(node * l, node * r) {
  node * ll = splitLeft(l);
  node * rr = splitRight(r);
  data d = r->t;
  merge(r, rr, true);
  merge(ll, l, false);
  return d;
...
void apply(node * l, node * r, operation o) {
  node * ll = splitLeft(l);
  node * rr = splitRight(r);
  r->o = combine(o, r->o);
  merge(r, rr, true);
...
int main() {
  ...
  for (int i = 0; i < n; i++)
    v[i] = new node();
  for (int i = 0; i < n; i++)
    cin >> v[i]->t.o, v[i]->t.v = v[i]->t.o;
  for (int i = 0; i < q; i++) {
    cin >> t >> l >> r, l--, r--;
    if (t == 1) {
      cin >> f >> o;
      applyPath(v[l], v[r], {f, o});
    } else if (t == 2) {
      data d = queryPath(v[l], v[r]);
      cout << d.v << "\n";
    } else if (t == 3) {
      link(v[l], v[r]);
    } else {
